...
fn three_spheres() -> (Camera, World, Vector3<f32>) {
    ...
    world.push(Sphere::new(Vector3::new(-1.0, 0.0, -1.0), -0.40, mat_left));

    world.push(Sphere::new(Vector3::new(1.0, 0.0, -1.0), 0.5, mat_right));
...
fn two_perlin_spheres(checker: bool) -> (Camera, World, Vector3<f32>) {
    let aspect_ratio = 16.0 / 9.0;
...
fn simple_light() -> (Camera, World, Vector3<f32>) {
    let aspect_ratio = 16.0 / 9.0;
...
fn cornell_box_smoke(smoke: bool) -> (Camera, World, Vector3<f32>) {
    ...
    world.push(RectAA::xz(113.0, 443.0, 127.0, 432.0, 554.0, light.clone()));
...
fn final_scene() -> (Camera, World, Vector3<f32>) {
    let aspect_ratio = 1.0;
    ...
    let light = DiffuseLight::new(SolidColor::new(7.0, 7.0, 7.0));
    world.push(RectAA::xz(123.0, 423.0, 147.0, 412.0, 554.0, light));
    ...
    world.push(Sphere::new(
        ...
        50.0,
        Metal::new(Vector3::new(0.8, 0.8, 0.9), 1.0),
    ));
